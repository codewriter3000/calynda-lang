...
export const TYPE_DOCS: Record<string, TypeInfo> = {
  ...
  num: { name: 'num', description: 'Generic numeric placeholder resolved per call site by the normal numeric widening rules.', examples: ['num twice = (num value) -> value + value;'] },
  ...
  mmio: { name: 'mmio<T>', description: 'Typed volatile memory-mapped I/O handle. Supports `.value` loads/stores, typed `offset`, and address-oriented cache helpers.', examples: ['manual { mmio<uint32> reg = 0x4000_0000; reg.value = 1; };', 'cacheclean(reg);'] },
