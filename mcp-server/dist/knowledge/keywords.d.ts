export declare const KEYWORDS: readonly ["package", "import", "public", "private", "final", "var", "start", "boot", "return", "exit", "throw", "null", "true", "false", "void", "export", "as", "internal", "static", "thread_local", "type", "union", "manual", "arr", "ptr", "mmio", "layout", "spawn", "checked", "asm", "malloc", "calloc", "realloc", "free", "deref", "store", "offset", "addr", "cleanup", "stackalloc"];
...
export declare const KEYWORD_DOCS: {
    ...
    readonly mmio: "Typed memory-mapped I/O handle. `mmio<T>` behaves like a volatile device address: `.value` reads/writes as `T`, `offset` preserves typed strides, and cache/barrier helpers accept it where an address is required.";
...
export declare const BUILTIN_CALL_DOCS: {
    readonly fence: "`fence()` returns `void` and emits the runtime/device barrier helper. It accepts no arguments.";
    readonly cacheclean: "`cacheclean(address)` returns `void` and accepts an integral address, `ptr<T>`, or `mmio<T>` handle.";
    readonly cachefinal: "`cachefinal()` returns `void`, accepts no arguments, and finalizes a cache-maintenance sequence.";
    readonly typeof: "`typeof(value)` returns a `string` naming the runtime type. Dispatched through the runtime, not constant-folded.";
    readonly isint: "`isint(value)` returns `bool`. Companion to `isfloat`, `isbool`, `isstring`, `isarray`.";
    readonly issametype: "`issametype(x, y)` returns `bool` — true iff x and y share a runtime type tag.";
    readonly car: "`car(arr)` returns the first element of a non-empty array. **alpha.6** also accepts `string`, returning the first byte as `char`. Aborts at runtime on empty input.";
...
export declare const PARAMETER_FORMS: readonly [{
    readonly syntax: "(Type name) -> ...";
    readonly description: "Standard typed parameter.";
}, {
    readonly syntax: "(Type... name) -> ...";
