...
export function explainTopic(input: ExplainInput): ExplainResult {
  ...
  const keywordExplanations: Record<string, string> = {
    lambda: 'Calynda uses lambdas for all functions. Syntax: `(type param) -> expr` or `(type param) -> { ... }`. Whole-function manual shorthand is also supported for block-bodied lambdas: `manual(type param) -> { ... }`. Lambda types collapse to their return type. Closures capture outer locals/parameters by symbol identity.',
