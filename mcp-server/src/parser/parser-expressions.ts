...
import { ParserState, ParseError, ASSIGNMENT_OPS } from './parser';
...
export function parseExpression(state: ParserState): AST.Expression {
  if (isManualLambdaAhead(state)) return parseManualLambdaExpression(state);
  if (isLambdaAhead(state)) return parseLambdaExpression(state);
  return parseAssignmentExpression(state);
}

function isManualLambdaAhead(state: ParserState): boolean {
  if (!state.check('keyword', 'manual') || state.peek(1).type !== 'lparen') return false;
  let i = 2;
  let depth = 1;
...
function isLambdaAhead(state: ParserState): boolean {
  if (!state.check('lparen')) return false;
  let i = 1;
  let depth = 1;
  while (state.pos + i < state.tokens.length && depth > 0) {
    const t = state.tokens[state.pos + i];
...
function parseLambdaExpression(state: ParserState): AST.LambdaExpression {
...
function parseManualLambdaExpression(state: ParserState): AST.LambdaExpression {
  const startPos = state.position();
  ...
  if (!state.check('lbrace')) {
    ...
    throw new ParseError('manual lambda shorthand requires a block body', tok.line, tok.column);
  }
  const body = parseBlock(state);
  return { kind: 'LambdaExpression', params, body, start: startPos, end: state.position() };
}

function parseAssignmentExpression(state: ParserState): AST.Expression {
  const left = parseTernaryExpression(state);
  const tok = state.peek();
...
export function parsePostfix(state: ParserState): AST.Expression {
...
function parsePrimary(state: ParserState): AST.Expression {
  ...
  if (tok.type === 'integer') {
    ...
    let val: number;
    if (normalized.startsWith('0b') || normalized.startsWith('0B')) val = parseInt(normalized.slice(2), 2);
    else if (normalized.startsWith('0x') || normalized.startsWith('0X')) val = parseInt(normalized.slice(2), 16);
    else if (normalized.startsWith('0o') || normalized.startsWith('0O')) val = parseInt(normalized.slice(2), 8);
    else val = parseInt(normalized, 10);
...
function parseTemplateParts(raw: string): (string | AST.Expression)[] {
  ...
  while (i < content.length) {
    if (content[i] === '$' && content[i + 1] === '{') {
      ...
      try {
        ...
        const innerState = new ParserState(innerTokens);
