...
static bool build_invalid_machine_program(MachineProgram *machine_program,
                                          char *expected,
...
void test_asm_emit_preserves_upstream_error_spans(void) {
    ...
    REQUIRE_TRUE(build_invalid_machine_program(&machine_program, expected, sizeof(expected)),
                 "build structured machine error for asm emit precondition");
    ASSERT_TRUE(asm_emit_program_to_string(&machine_program) == NULL,
                "asm emit rejects machine programs with structured errors");
    REQUIRE_TRUE(asm_emit_get_error(&machine_program, &error),
                 "asm emit exposes structured error for invalid machine input");
    REQUIRE_TRUE(asm_emit_format_error(&error, actual, sizeof(actual)),
                 "format propagated asm emit error");
    ASSERT_TRUE(strcmp(expected, actual) == 0,
                "asm emit error preserves upstream diagnostic text");
    ASSERT_TRUE(error.primary_span.start_line ==
                    machine_get_error(&machine_program)->primary_span.start_line &&
                error.primary_span.start_column ==
                    machine_get_error(&machine_program)->primary_span.start_column,
                "asm emit error preserves the primary diagnostic span");
