...
void test_lir_dump_lowers_hetero_array_literal(void) {
    ...
    REQUIRE_TRUE(parser_parse_program(&parser, &ast_program), "parse hetero array LIR program");
    REQUIRE_TRUE(symbol_table_build(&symbols, &ast_program), "build symbols for hetero array LIR program");
    REQUIRE_TRUE(type_checker_check_program(&checker, &ast_program, &symbols),
                 "type check hetero array LIR program");
    REQUIRE_TRUE(hir_build_program(&hir_program, &ast_program, &symbols, &checker),
                 "lower HIR for hetero array LIR program");
    REQUIRE_TRUE(mir_build_program(&mir_program, &hir_program, false),
                 "lower MIR for hetero array LIR program");
    REQUIRE_TRUE(lir_build_program(&lir_program, &mir_program),
                 "lower LIR for hetero array literal");

    dump = lir_dump_program_to_string(&lir_program);
    REQUIRE_TRUE(dump != NULL, "render hetero array LIR dump to string");
    ASSERT_CONTAINS("hetero_array_new typedesc(arr|1|g0:raw_word|0:int32|1:bool|2:string) [int32(1), bool(true), string(\"hello\")]",
                    dump,
                    "arr<?> literals carry descriptor metadata through LIR");
...
void test_lir_dump_lowers_union_descriptors(void) {
    ...
    lir_program_init(&lir_program);
    parser_init(&parser, source);
    REQUIRE_TRUE(parser_parse_program(&parser, &ast_program), "parse union LIR program");
    REQUIRE_TRUE(symbol_table_build(&symbols, &ast_program), "build symbols for union LIR program");
    REQUIRE_TRUE(type_checker_check_program(&checker, &ast_program, &symbols),
                 "type check union LIR program");
    REQUIRE_TRUE(hir_build_program(&hir_program, &ast_program, &symbols, &checker),
                 "lower HIR for union LIR program");
    REQUIRE_TRUE(mir_build_program(&mir_program, &hir_program, false),
                 "lower MIR for union LIR program");
    REQUIRE_TRUE(lir_build_program(&lir_program, &mir_program),
                 "lower LIR for union descriptors");

    dump = lir_dump_program_to_string(&lir_program);
    REQUIRE_TRUE(dump != NULL, "render union LIR dump to string");
    ASSERT_CONTAINS("union_new typedesc(Option|1|g0:int32|Some:int32|None:void) variant 0 payload int32(42)",
                    dump,
                    "union payload constructors carry specialized descriptors through LIR");
    ASSERT_CONTAINS("union_new typedesc(Option|1|g0:raw_word|Some:raw_word|None:void) variant 1",
                    dump,
                    "union non-payload constructors keep descriptor metadata through LIR");
