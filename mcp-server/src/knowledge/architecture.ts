...
import { PIPELINE_STAGES_FRONTEND } from './architecture-stages';
...
const PIPELINE_STAGES_BACKEND = [
  ...
  },
  {
    ...
    description:
      'Target-aware code generation via TargetDescriptor abstraction (x86_64 SysV ELF and AArch64 AAPCS64 ELF). Classifies LIR ops into direct machine-pattern candidates vs runtime helpers. Uses target-specific register sets for allocation.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Target-agnostic machine layer consuming LIR + codegen plan. Emits instruction streams, helper-call setup, prologue/epilogue, and conservative register preservation. All operations use TargetDescriptor for register names and ABI conventions. Supports x86_64 and AArch64.',
  ...
  },
  {
    ...
    dir: 'compiler/src/backend/',
    description:
      'Lowers MachineProgram into GNU assembler text with concrete stack addresses, rodata literals, global storage, string-object data, runtime helper calls, main entry glue, and .note.GNU-stack. Dispatches to x86_64 or AArch64 emitters based on target.',
    keyTypes: [],
  ...
  },
  {
    ...
    description:
      'Command-line tools: the calynda compiler driver (supports --target flag for x86_64/aarch64), AST dumper, semantic dumper, assembly emitter, bytecode emitter, native builder, and CAR archive commands (pack/build/run). The native builder resolves runtime.o relative to the executable directory.',
    keyTypes: [],
    keyFunctions: [],
    files: [
...
export const PIPELINE_STAGES = [...PIPELINE_STAGES_FRONTEND, ...PIPELINE_STAGES_BACKEND] as const;
