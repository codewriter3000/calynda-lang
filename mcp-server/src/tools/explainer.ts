...
export function explainTopic(input: ExplainInput): ExplainResult {
  ...
  const keywordExplanations: Record<string, string> = {
    ...
    start: 'The `start` function is the program entry point. Syntax: `start(string[] args) -> { ... };` or the bare form `start -> { ... };` (no parameters). It implicitly returns int32 (the exit code). The compiler enforces exactly one `start`.',
    ...
    mmio: '`mmio<T>` is the typed MMIO/device-memory surface. It behaves like a volatile address with a known element type: `offset(mmio, n)` uses typed strides, `.value` reads/writes as `T`, and cache helpers accept the handle as an address source.',
    ...
    varargs: 'Variadic parameters use `Type... name` syntax. Must be the last parameter. Parameters may also carry an optional default value: `Type name = defaultExpr`. Defaults must appear after required parameters.',
    'default parameter': 'Parameters can carry default values using `= expression` after the name. Example: `(int32 left, int32 right = left + 1) -> left + right`. Default values must appear after non-default parameters.',
    swap: 'The swap statement `a >< b;` atomically exchanges the values of two assignable l-values of the same type in a single statement. Both targets must be mutable and non-final. HIR lowers a swap into a temp-local plus two assignment statements. Example: `values[0] >< values[2];`.',
    'sized array': 'Declared fixed-size arrays such as `int32[4]` and `int32[2][3]` are now checked semantically where extent metadata survives: binding initializers, assignments, call boundaries, parameter defaults, lambda returns, and ternary merges reject mismatched shapes.',
    ...
    'inline asm': 'Block bodies also accept statement-level inline assembly: `asm { ... };`. The compiler lowers each statement by synthesizing a hidden zero-argument asm unit and then calling it from the surrounding function or `boot` body.',
    asm: '`asm()` is the top-level inline assembly declaration form. Syntax: `int32 name = asm(int32 a, int32 b) -> { mov eax, edi; add eax, esi; ret };`. Block bodies also accept statement-level `asm { ... };` forms. Assembly text is passed through to the assembler unchanged, parameters follow normal Calynda type syntax, and the `export` / `static` modifiers are supported on declarations.',
    boot: '`boot` is a bare-metal entry point that bypasses the Calynda runtime and emits a freestanding `_start` symbol. Intended for freestanding and embedded targets. Syntax: `boot -> 0;` or `boot -> { ... };`. `boot` and `start` cannot coexist, and the contract does not advertise Linux-only exit behaviour for bare-metal entry glue.',
    ...
    fence: '`fence()` is the zero-argument barrier builtin for freestanding/device code. It returns `void` and lowers to the dedicated runtime/helper hook for target-specific memory ordering.',
    cacheclean: '`cacheclean(address)` performs cache-line cleaning for an integral address, `ptr<T>`, or `mmio<T>` handle. It returns `void` and is intended for bare-metal / low-level runtime coordination.',
    cachefinal: '`cachefinal()` completes a cache-maintenance sequence and returns `void`.',
    ...
    gc: 'Hosted compilation now exposes `--gc marksweep|legacy` plus `--gc-plugin path.a`. The default hosted runtime archive is mark-and-sweep (`calynda_runtime_ms.a`); `--gc legacy` selects the older append-only registry, and `--gc-plugin` links a user-supplied archive that exports the GC backend interface.',
    'manual-bounds-check': 'The `--manual-bounds-check` CLI flag forces manual-memory operations through the checked helper path, even outside `manual checked { ... };` blocks.',
