...
export const PIPELINE_STAGES = [
  {
    ...
    description:
      'Lexes source text into a flat token stream. Tracks nested braces inside template interpolations so expressions like lambdas with block bodies parse correctly inside templates.',
  ...
  },
  {
    ...
    description:
      'Recursive-descent parser with token pre-loading for lookahead. Parses the full EBNF grammar into an AST. Handles named types, generic arguments, >> splitting for nested generics, and all V2 surface features.',
  ...
  },
  {
    ...
    description:
      'Abstract syntax tree node definitions and creation helpers. Declarations, expressions, statements, and types all carry source spans for diagnostic reporting. AST dump utilities provide both FILE-based and string-returning APIs.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Builds a program scope plus nested start/lambda/block scopes. Tracks package/import/top-level/parameter/local symbols, records identifier resolutions, and reports unresolved names. Symbols carry is_exported, is_static, is_internal flags. Symbol kinds include SYMBOL_KIND_UNION and SYMBOL_KIND_TYPE_PARAMETER.',
  ...
  },
  {
    ...
    dir: 'compiler/src/sema/',
    description:
      'Resolves declared source types for bindings, parameters, locals, and casts. Validates array dimensions and rejects invalid void usage before type checking.',
  ...
  },
  {
    ...
    dir: 'compiler/src/sema/',
    description:
      'Renders package/imports, hierarchical scopes, declared and inferred types, identifier resolution edges, unresolved names, and explicit shadowing notes.',
    keyTypes: [],
  ...
  },
  {
    ...
    description:
      'Lowers typed AST + semantic state into an owned, typed IR. Normalizes expression-bodied lambdas and start bodies into block form, erases grouping expressions, normalizes exit; into return;, and owns callable signatures. Local bindings preserve callable metadata.',
  ...
  },
  {
    ...
    description:
      'Machine-independent IR with explicit callable units, locals, temporaries, basic blocks, branch/goto/return/throw terminators. Lowers short-circuit &&/|| into branch/join blocks, ternary into merge locals, closures into MIR_UNIT_LAMBDA + MIR_INSTR_CLOSURE, and top-level bindings into __mir_module_init.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Target-aware IR with stack slots, virtual registers, explicit incoming capture/argument moves, explicit outgoing call-argument setup, and block terminators.',
  ...
  },
  {
    ...
    description:
      'x86_64 SysV ELF target with register allocation. Classifies LIR ops into direct machine-pattern candidates vs runtime helpers. Allocatable vreg set: r10/r11/r12/r13 before spilling.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Consumes LIR + codegen plan to emit x86_64 SysV instruction streams. Handles helper-call setup, prologue/epilogue, and conservative register preservation around calls. Emits inc/dec for pre-increment/decrement.',
  ...
  },
  {
    ...
    dir: 'compiler/src/backend/',
    description:
      'Lowers MachineProgram into GNU assembler x86_64 text with concrete stack addresses, rodata literals, global storage, string-object data, runtime helper calls, main entry glue, and .note.GNU-stack.',
    keyTypes: [],
  ...
  },
  {
    ...
    dir: 'compiler/src/backend/',
    description:
      'Defines the x86_64 SysV helper-call surface for closures, callable dispatch, member/index access, arrays, templates, dynamic casts, throw, and capture-environment contract in r15.',
  ...
  },
  {
    ...
    description:
      'Concrete runtime objects and values. Values are raw machine words or registered heap-object handles. Object kinds: strings, arrays, closures, packages, extern callables, template-part packs, and unions. Includes process startup that boxes argv into Calynda string[] and registers static string objects.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Portable-v1 bytecode backend. Lowers MIR directly into BytecodeProgram structures with a constant pool, bytecode units, basic blocks, and explicit instructions + terminators. Mirrors the MIR opcode surface.',
    keyTypes: [
  ...
  },
  {
    ...
    description:
      'Command-line tools: the calynda compiler driver, AST dumper, semantic dumper, assembly emitter, bytecode emitter, and native builder. The native builder resolves runtime.o relative to the executable directory.',
    keyTypes: [],
    keyFunctions: [],
    files: [
...
export const BYTECODE_PIPELINE =
  'Source → Tokenizer → Parser → AST → SymbolTable → TypeResolution → TypeChecker → HIR → MIR → BytecodeProgram (portable-v1)';

export const BACKEND_STRATEGY = `Calynda has two compiler backends and rejects any interpreter path:
...
export const SOURCE_TREE = `compiler/
...
export const ERROR_PATTERN = `Each compiler stage uses the same error struct pattern:
