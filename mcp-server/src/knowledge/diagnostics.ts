...
export function findDiagnosticDocs(query: string): DiagnosticDoc[] {
  ...
  return DIAGNOSTIC_DOCS.filter(doc => {
...
export function formatDiagnosticDoc(doc: DiagnosticDoc): string {
  const lines: string[] = [
    `## ${doc.title}`,
    `- Kind: ${doc.kind}`,
    `- Summary: ${doc.summary}`,
